...
/**
 * Wrapper for standard HBase Admin object; in addition to standard Admin functionality,
 * an MAdmin object interfaces with the ColumnManager repository for tracking of metadata changes.
...
class MAdmin implements Admin {
    ...
    /** In HBase 2.0, this method will return a Future object, which can be
     * handed off to another thread to await completion of the table creation
     * and then invoke the appropriate Repository logic.
     * For now, this method not supported when Repository is ACTIVATED.
     * @param htd
     * @param bytes
    ...
    public HTableDescriptor[] deleteTables(String tableNameRegex) throws IOException {
        if (repository.isActivated())
            throw new UnsupportedOperationException
                (UNSUPPORTED_WHEN_COLMANAGER_ACTIVATED_MSG + UNSUPPORTED_MASS_DISABLE_DELETE_METHOD);
