 ...
 * ColumnDefinitionsEnforced} setting {@code enabled}, then (a) any <i>Column Qualifier</i> submitted in a
 * {@code put} (i.e., insert/update) to the <i>Table:Column-Family</i> must correspond to an existing
 * {@code ColumnDefinition}, and (b) the corresponding
 * <i>Column value</i> submitted must pass all validations (if any) stipulated by the {@code ColumnDefinition}.
 */
...
//    static final String NAME_IS_REGEX_KEY = "NAME_IS_REGEX";
    ...
    /**
     * This constructor accessed during deserialization process (i.e., building of
     * objects by pulling metadata components from Repository or from external archive).
     ...
     */
    @Override
    final ColumnDefinition setValue
    ...
    /**
     * Set the defined column length (i.e., the maximum length permitted for any submitted
     * value for this column when
     * {@link RepositoryAdmin#setColumnDefinitionsEnforced(boolean, org.apache.hadoop.hbase.TableName, byte[])
     * ColumnDefinitionsEnforced} is set to {@code true} for the column's <i>Column Family</i>);
     * setting the value to 0 (the default) will result in NO length validation of column values
     * being performed.
    ...
    /**
     * Get the defined column length (i.e., the maximum length permitted for any submitted
     * value for this column when
     * {@link RepositoryAdmin#setColumnDefinitionsEnforced(boolean, org.apache.hadoop.hbase.TableName, byte[])
     * ColumnDefinitionsEnforced} is set to {@code true} for the column's <i>Column Family</i>);
     * if returned value is 0 (the default), NO length validation of column values will be performed.
    ...
    /**
     * Set the column's validation regex (i.e., the regular expression that any
     * submitted value for this column must match when
     * {@link RepositoryAdmin#setColumnDefinitionsEnforced(boolean, org.apache.hadoop.hbase.TableName, byte[])
     * ColumnDefinitionsEnforced} is set to {@code true} for the column's <i>Column Family</i>);
    ...
    /**
     * Get the defined column regex (i.e., the regular expression that any
     * submitted value for this column must match when
     * {@link RepositoryAdmin#setColumnDefinitionsEnforced(boolean, org.apache.hadoop.hbase.TableName, byte[])
     * ColumnDefinitionsEnforced} is set to {@code true} for the column's <i>Column Family</i>);
     * if returned value is blank (the default), NO regular expression validation of column values will
     * be performed.
     * @return defined column length for this column
     */
