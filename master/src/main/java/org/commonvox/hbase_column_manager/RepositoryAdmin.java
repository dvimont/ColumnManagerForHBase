...
/**
 * A <b>RepositoryAdmin</b> provides ColumnManager repository maintenance and query
 * facilities, as well as metadata {@link #discoverMetadata() discovery},
 * {@link #exportNamespaceMetadata(java.lang.String, java.lang.String, java.lang.String, boolean)
 * export}, and {@link #importMetadata(boolean, java.lang.String, java.lang.String) import} facilities;
 * it is used as a complement to the standard {@code Admin} interface, with an
 ...
 * {@code RepositoryAdmin} instance being used to maintain {@link ColumnDefinition}
 * structures and to query {@link ColumnAuditor} structures.
...
public class RepositoryAdmin implements Closeable {
    ...
    /**
     * Initialize a RepositoryAdmin object using an already-created Connection.
     * The Connection will NOT be closed when this RepositoryAdmin object is closed.
    ...
    /**
     * Create namespace and table structures for Repository; note that
     * these structures will automatically be created with the first
     * invocation of {@link MConnectionFactory#createConnection()} if the
    ...
    public static void uninstallRepositoryStructures (Admin hbaseAdmin) throws IOException {
        Repository.dropRepository
                (hbaseAdmin, Logger.getLogger(RepositoryAdmin.class.getPackage().getName()));
    }

    /**
     * Causes the complete contents (all cells) of the Repository table to be "dumped"
     * (written) to the logging facility, for maintenance or debugging purposes.
     * @throws IOException if a remote or network exception occurs
     */
     ...
     * @return set of {@link ColumnAuditor}s, or null if <i>Table</i> or <i>Column Family</i> not found
     * @throws IOException if a remote or network exception occurs
     */
    ...
    /**
     * Get Column Qualifiers from the {@link ColumnAuditor} metadata in the
     * repository for a specified <i>Table</i> and <i>Column Family</i>.
     * @param htd Table Descriptor
     * @param hcd Column [Family] Descriptor
    ...
    /**
     * Add (or modify, if already existing) the submitted {@link ColumnDefinition} to
     * the submitted <i>Table</i> and <i>Column Family</i>.
     ...
     * @param hcd <i>ColumnAuditor [Family] Descriptor</i> to which {@link ColumnDefinition}
     * is to be added
     * @param colDefinition {@link ColumnDefinition} to be added or modified
     * @throws IOException if a remote or network exception occurs
     */
    ...
    /**
     * Add (or modify, if already existing) the submitted {@link ColumnDefinition}s to
     * the submitted <i>Table</i> and <i>Column Family</i>.
     ...
     * @param hcd <i>ColumnAuditor [Family] Descriptor</i> to which {@link ColumnDefinition}s
     * are to be added
     * @param colDefinitions List of {@link ColumnDefinition}s to be added or modified
     * @throws IOException if a remote or network exception occurs
     */
    ...
    public Collection<ColumnDefinition> getColumnDefinitions
                                            (HTableDescriptor htd, HColumnDescriptor hcd)
            throws IOException {
        if (MColumnDescriptor.class.isAssignableFrom(hcd.getClass())) {
     ...
     */
    public Collection<ColumnDefinition> getColumnDefinitions
                                    (TableName tableName, byte[] colFamily)
            throws IOException {
        MTableDescriptor mtd = getRepository().getMTableDescriptor(tableName);
        MColumnDescriptor mcd = mtd.getMColumnDescriptor(colFamily);
     ...
     * @param colQualifier qualifier which uniquely identifies the {@link ColumnDefinition}
     * to be deleted
     * @throws IOException if a remote or network exception occurs
     */
     ...
     * @param hcd <i>ColumnAuditor [Family] Descriptor</i> from which {@link ColumnDefinition}
     * is to be deleted
     * @param colQualifier qualifier which uniquely identifies {@link ColumnDefinition}
     * to be deleted
     * @throws IOException if a remote or network exception occurs
     */
    ...
    /**
     * Enable/disable column definition enforcement for the specified <i>Column Family</i>.
     * When <a href="package-summary.html#activate">ColumnManager is activated</a> and
     * column definition enforcement is enabled for a <i>Column Family</i>
     * of a <i>Table</i> that is <a href="package-summary.html#config">included in ColumnManager processing</a>,
     * then any <i>Column qualifier</i> submitted in a {@code Mutation} to the <i>Table</i> and
     * <i>Column Family</i> (via the HBase API) must correspond to an existing
     * {@link ColumnDefinition}, and the corresponding <i>Column value</i> submitted must pass
     * all validations (if any) stipulated by the {@code ColumnDefinition}.
     * @param enabled if {@code true}, column definition enforcement is enabled;
     * if {@code false}, it is disabled.
...
//        }
     ...
     * <b>SPECIAL NOTE:</b> An HTableMultiplexer returned by this method will
     * (1) validate submitted <i>Column</i> qualifiers and values
     * (if {@link RepositoryAdmin#setColumnDefinitionsEnforced(boolean, org.apache.hadoop.hbase.TableName, byte[])
     * ColumnDefinitionsEnforced} is set to {@code true} for the related <i>Column Family</i>),
     * (2) process "put" requests in a standard manner
     * (queuing them for subsequent <b>asynchronous</b> processing by HBase) and then
     * (3) perform <b>synchronous</b> ColumnManager repository processing upon the metadata of
     * each successfully queued "put". <i>Be advised that the third step will not take
     * into account any potentially failed "put" transactions among those queued in
     * the second step; instead it assumes that all "put" transactions succeeded, updating
     * the ColumnManager repository accordingly.</i>
     * @param perRegionServerBufferQueueSize determines the max number of the
     * buffered Put ops for each region server before dropping a request.
    ...
    /**
     * Performs discovery of all ColumnManager-included user <i>Table</i>s and stores the metadata
     * in the ColumnManager repository; includes discovery of {@link ColumnAuditor} metadata (performing full scan
     * of all <i>Table</i>s that are
    ...
    /**
     * Performs discovery of the specified <i>Table</i>'s metadata and stores the metadata
     * in the ColumnManager repository; includes discovery of {@link ColumnAuditor} metadata
     * (performing full scan of the <i>Table</i>). Note that the specified <i>Table</i>
    ...
    /**
     * Creates an external HBaseSchemaArchive (HSA) file in XML format* containing the
     * complete metadata contents (i.e., all <i>Namespace</i>, <i>Table</i>, <i>Column Family</i>,
     * {@link ColumnAuditor}, and {@link ColumnDefinition} metadata) of the ColumnManager metadata repository.
     * In effect, this is an XML-formatted serialization of objects of the following
     ...
     * @param formatted if <b>true</b>, insert whitespace (linefeeds and
     * hierarchical indentations) between XML elements to produce human-readable XML.
     * @throws IOException if a remote or network exception occurs
    ...
    /**
     * Creates an external HBaseSchemaArchive (HSA) file in XML format* containing the
     * complete metadata contents (i.e., the <i>Namespace</i>, <i>Table</i>, <i>Column Family</i>,
     * {@link ColumnAuditor}, and {@link ColumnDefinition} metadata) of the specified HBase <i>Namespace</i>.
     * In effect, this is an XML-formatted serialization of objects of the following
    ...
    /**
     * Creates an external HBaseSchemaArchive (HSA) file in XML format* containing the
     * complete metadata contents (i.e., the <i>Table</i>, <i>Column Family</i>,
     * {@link ColumnAuditor}, and {@link ColumnDefinition} metadata) of the specified HBase <i>Table</i>.
     * In effect, this is an XML-formatted serialization of objects of the following
     * classes: {@link MTableDescriptor}, {@link MColumnDescriptor}, {@link ColumnAuditor},
     * and {@link ColumnDefinition}.
     * <br><br>*An HSA file adheres to the XML Schema layout in
     * <a href="doc-files/HBaseSchemaArchive.xsd.xml" target="_blank">HBaseSchemaArchive.xsd.xml</a>.
    ...
    /**
     * Import into HBase the complete contents of an external HBaseSchemaArchive (HSA)
     * XML file*. This process will NOT overlay any existing Namespace and Table
     * structures in HBase.
     * For any Namespace definition in the external file which does NOT correspond to
     * an existing HBase Namespace, a new Namespace will be added to HBase along with
     * any Tables and table components belonging to the Namespace. For any Namespace
     * definition in the external file which corresponds to an existing HBase Namespace,
     * any of its Table and table components will only be added to HBase if the Table
     * does NOT already exist in HBase.
     * <br><br>*An HSA file adheres to the XML Schema layout in
     * <a href="doc-files/HBaseSchemaArchive.xsd.xml" target="_blank">HBaseSchemaArchive.xsd.xml</a>.
     * @param includeColumnAuditors if <b>true</b>, import {@link ColumnAuditor}
     * metadata from the HBaseSchemaArchive file into the ColumnManager repository.
    ...
    /**
     * Import into HBase a Namespace and all its component objects as represented
     * in an external HBaseSchemaArchive (HSA) XML file*.
     * This process will NOT overlay any existing Namespace and Table
     * structures in HBase.
     * If the Namespace definition in the external file does NOT correspond to
     * an existing HBase Namespace, a new Namespace will be added to HBase along with
     * any Tables and table components belonging to the Namespace. If the Namespace
     * definition in the external file corresponds to an existing HBase Namespace, any
     * of its Table and table components will only be added to HBase if the Table does
     * NOT already exist in HBase.
     * <br><br>*An HSA file adheres to the XML Schema layout in
     * <a href="doc-files/HBaseSchemaArchive.xsd.xml" target="_blank">HBaseSchemaArchive.xsd.xml</a>.
     * @param includeColumnAuditors if <b>true</b>, import {@link ColumnAuditor}
     * metadata from the HBaseSchemaArchive file into the ColumnManager repository.
    ...
    /**
     * Import into HBase a Table and all its component objects as represented
     * in an external HBaseSchemaArchive (HSA) XML file*.
     * This process will NOT overlay any existing Namespace and Table
     * structures in HBase.
     * If the Table's parent Namespace does NOT correspond to an existing HBase
     * Namespace, a new Namespace will be added to HBase. The Table and table
     * components will only be added to HBase if the Table does NOT already exist
     * in HBase.
     * <br><br>*An HSA file adheres to the XML Schema layout in
     * <a href="doc-files/HBaseSchemaArchive.xsd.xml" target="_blank">HBaseSchemaArchive.xsd.xml</a>.
     * @param includeColumnAuditors if <b>true</b>, import {@link ColumnAuditor}
    ...
    /**
     * Generates a hierarchically-indented, text-based summary report of the
     * contents of an external HBaseSchemaArchive (HSA) XML file*.
     * <br><br>*An HSA file adheres to the XML Schema layout in
     * <a href="doc-files/HBaseSchemaArchive.xsd.xml" target="_blank">HBaseSchemaArchive.xsd.xml</a>.
     ...
     * @throws JAXBException if an exception occurs in the context of JAXB processing
     */
    public String generateHmaFileSummary
                (String sourcePathString, String sourceFileNameString)
    ...
    public boolean namespaceExists (String namespaceName)
            throws IOException {
        try { repository.getAdmin().getNamespaceDescriptor(namespaceName);
