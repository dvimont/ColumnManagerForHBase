...
/**
 * Extension of standard HTableMultiplexer class which (in addition to providing
 * all superclass functionality) transparently submits Column metadata
 * to the ColumnManager repository.
...
class MTableMultiplexer extends HTableMultiplexer {
     ...
     * buffered Put ops for each region server before dropping the request.
     * @throws IOException
     */
    ...
    /**
     * The put requests will be buffered by their corresponding buffer queues.
     * Returns the list of puts which could not be queued.
