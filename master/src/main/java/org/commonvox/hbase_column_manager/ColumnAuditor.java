 ...
 * {@code ColumnAuditor} metadata is gathered at runtime
 * as {@code Mutation}s are submitted via the HBase API to any
 * <a href="package-summary.html#config">ColumnManager-included</a> <i>Table</i>.
 * <br><br>
 * **{@code ColumnAuditor} metadata may also be gathered for already-existing <i>Column</i>s
 * via the {@link RepositoryAdmin} method {@link RepositoryAdmin#discoverMetadata() discoverMetadata}.
 */
public class ColumnAuditor extends Column {
     ...
     * Note that MaxValueLengthFound for a given column is incremented upward only
     * when a value longer than that of any previously submitted/discovered value
     * is found (either in the context of real-time metadata capture from a submitted
     * Table {@code Mutation}, or in the process of
     * {@link RepositoryAdmin#discoverMetadata() metadata discovery}).
     * MaxValueLengthFound is never decremented, even when the longest
     * value for the column is deleted from HBase, so the MaxValueLengthFound
     * actually represents the maximum length EVER recorded for the value of
     * a specific column, not necessarily the longest value CURRENTLY stored
     * in HBase for the column.
